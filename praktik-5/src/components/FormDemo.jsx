...
const FormDemo = () => {
  ...
  const handleBlur = (event) => {
    ...
    // Jalankan validasi setelah touch
    validateField(name, formData[name]);
  };

  // Validasi individual field (opsional, untuk menampilkan error onBlur)
  ...
  // Validasi Form Keseluruhan (dipanggil saat submit)
  const validateForm = () => {
    const newErrors = {};
  ...
  const handleSubmit = (event) => {
    ...
    // Set all fields to touched to show errors on all required fields
  ...
  const hasError = (fieldName) => {
    // Tambahkan kondisi untuk error pada radio/select agar langsung muncul saat submit
    if (isSubmitted && errors[fieldName] && (fieldName === 'gender' || fieldName === 'jurusan')) {
        return true;
    }
